...
export default function SpecializedStaffFields({ form, setForm, services = [] }) {
  ...
  return <div className="rounded-2xl border border-emerald-100 bg-emerald-50/30 p-4 space-y-4">
    ...
    <div><p className="text-[10px] font-black uppercase tracking-widest text-slate-600 mb-2">Availability Schedule</p><div className="space-y-2">{Object.keys(defaultAvailability).map(day=>{const value=profile.availability?.[day] || defaultAvailability[day];const serviceBreak=value.breaks?.[0]||{start:'',end:''};return <div key={day} className="grid min-w-0 grid-cols-2 items-center gap-2 lg:grid-cols-[82px_repeat(4,minmax(0,1fr))]"><label className="col-span-2 flex items-center gap-2 text-[10px] font-bold capitalize lg:col-span-1"><input type="checkbox" checked={Boolean(value.available)} onChange={event=>updateProfile({availability:{...profile.availability,[day]:{...value,available:event.target.checked}}})}/>{day}</label><input aria-label={`${day} start`} title="Working hours start" type="time" disabled={!value.available} value={value.start || '09:00'} onChange={event=>updateProfile({availability:{...profile.availability,[day]:{...value,start:event.target.value}}})} className={`${inputClass} min-w-0 mt-0 disabled:opacity-40`}/><input aria-label={`${day} end`} title="Working hours end" type="time" disabled={!value.available} value={value.end || '17:00'} onChange={event=>updateProfile({availability:{...profile.availability,[day]:{...value,end:event.target.value}}})} className={`${inputClass} min-w-0 mt-0 disabled:opacity-40`}/><input aria-label={`${day} break start`} title="Break start (optional)" type="time" disabled={!value.available} value={serviceBreak.start || ''} onChange={event=>updateProfile({availability:{...profile.availability,[day]:{...value,breaks:event.target.value?[{...serviceBreak,start:event.target.value}]:[]}}})} className={`${inputClass} min-w-0 mt-0 disabled:opacity-40`}/><input aria-label={`${day} break end`} title="Break end (optional)" type="time" disabled={!value.available} value={serviceBreak.end || ''} onChange={event=>updateProfile({availability:{...profile.availability,[day]:{...value,breaks:event.target.value?[{...serviceBreak,end:event.target.value}]:[]}}})} className={`${inputClass} min-w-0 mt-0 disabled:opacity-40`}/></div>;})}</div><p className="mt-2 text-[9px] text-slate-400">Working start/end, followed by an optional break start/end. Booking assignment rejects shifts, breaks, leave, and unavailable periods.</p></div>
    ...
    <div><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-[10px] font-black uppercase tracking-widest text-slate-600">Leave Schedule</p><button type="button" onClick={()=>updateProfile({leaveSchedule:[...(profile.leaveSchedule||[]),{startDate:'',endDate:'',reason:''}]})} className="h-7 px-2 rounded-lg border bg-white text-[9px] font-black uppercase">Add Leave</button></div><div className="mt-2 space-y-2">{(profile.leaveSchedule||[]).map((leave,index)=><div key={index} className="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1.5fr_auto]"><input type="date" value={leave.startDate?String(leave.startDate).slice(0,10):''} onChange={event=>updateProfile({leaveSchedule:profile.leaveSchedule.map((item,i)=>i===index?{...item,startDate:event.target.value}:item)})} className={`${inputClass} min-w-0 mt-0`}/><input type="date" value={leave.endDate?String(leave.endDate).slice(0,10):''} onChange={event=>updateProfile({leaveSchedule:profile.leaveSchedule.map((item,i)=>i===index?{...item,endDate:event.target.value}:item)})} className={`${inputClass} min-w-0 mt-0`}/><input value={leave.reason||''} onChange={event=>updateProfile({leaveSchedule:profile.leaveSchedule.map((item,i)=>i===index?{...item,reason:event.target.value}:item)})} className={`${inputClass} min-w-0 mt-0 sm:col-span-2 lg:col-span-1`} placeholder="Reason"/><button type="button" onClick={()=>updateProfile({leaveSchedule:profile.leaveSchedule.filter((_,i)=>i!==index)})} className="h-9 w-full rounded-lg border text-rose-600 sm:col-span-2 lg:col-span-1 lg:w-9">×</button></div>)}</div></div>
  </div>;
}
