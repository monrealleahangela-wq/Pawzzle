...
export default function RiderDeliveryWorkspace({ delivery, token, eta, distanceKm, onStatusUpdate, onSendMessage, onRefresh }) {
  ...
  return <div className="min-h-[100dvh] bg-slate-50 pb-24 text-slate-900">
    ...
    <main className="max-w-3xl mx-auto p-4 space-y-4">
      ...
      {isCod && <section className="bg-amber-50 border border-amber-200 rounded-2xl p-4"><p className="text-[10px] uppercase font-black text-amber-700">COD amount</p><p className="text-2xl font-black text-amber-900">₱{Number(order.totalAmount).toLocaleString(undefined,{minimumFractionDigits:2})}</p></section>}
      ...
      <section className="bg-white border rounded-2xl p-4"><h2 className="text-sm font-black mb-3">Quick messages</h2><div className="flex gap-2 overflow-x-auto pb-1">{QUICK_MESSAGES.map(message=><button key={message} onClick={()=>quickMessage(message)} className="shrink-0 min-h-11 max-w-[190px] px-3 rounded-xl bg-slate-100 text-left text-xs font-bold"><Send size={13} className="inline mr-1"/>{message}</button>)}</div><button onClick={()=>setShowChat(true)} className="mt-3 text-xs font-black text-indigo-600">Custom message and history →</button></section>
    ...
    {delivery.isLive && <div className="fixed bottom-0 inset-x-0 z-40 bg-white border-t p-3"><div className="max-w-3xl mx-auto flex gap-2">{['pending','assigned','accepted'].includes(delivery.status)&&<button onClick={()=>window.confirm('Start this delivery?')&&onStatusUpdate('picked_up')} className="flex-1 h-12 rounded-xl bg-slate-900 text-white text-xs font-black"><Truck size={17} className="inline mr-2"/>Start delivery</button>}{delivery.status==='picked_up'&&<button onClick={()=>window.confirm('Begin delivery transit?')&&onStatusUpdate('in_transit')} className="flex-1 h-12 rounded-xl bg-indigo-600 text-white text-xs font-black">Out for delivery</button>}{delivery.status==='in_transit'&&<><button onClick={()=>setShowFailure(true)} className="h-12 px-4 rounded-xl border text-rose-600"><AlertTriangle size={18}/></button><button onClick={()=>window.confirm('Confirm arrival at the delivery location?')&&onStatusUpdate('arrived')} className="flex-1 h-12 rounded-xl bg-indigo-600 text-white text-xs font-black">Arrived at location</button></>}{delivery.status==='arrived'&&<><button onClick={()=>setShowFailure(true)} className="h-12 px-4 rounded-xl border text-rose-600"><AlertTriangle size={18}/></button><button onClick={()=>setShowProof(true)} className="flex-1 h-12 rounded-xl bg-emerald-600 text-white text-xs font-black">Confirm delivery</button></>}</div></div>}
