...
const InquiryModal = ({ isOpen, onClose, pet, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fade-in">
      <div className="bg-white w-full max-w-xl rounded-[2.5rem] overflow-hidden shadow-2xl animate-scale-up border border-slate-100 flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="bg-slate-900 p-6 sm:p-8 flex items-center justify-between relative overflow-hidden shrink-0">
          ...
          <div className="relative z-10">
            <h2 className="text-white text-xl sm:text-2xl font-black uppercase tracking-tight">Purchase Inquiry</h2>
            <p className="text-primary-400 text-[10px] font-black uppercase tracking-widest mt-1 italic">For {pet.name} • {pet.breed}</p>
          </div>
          ...
          <form id="inquiry-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold text-slate-900 focus:ring-2 focus:ring-primary-500/20 focus:bg-white transition-all outline-none"
                />
              </div>
              <div className="space-y-1.5">
                <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold text-slate-900 focus:ring-2 focus:ring-primary-500/20 focus:bg-white transition-all outline-none resize-none"
              />
            </div>
            ...
            {/* Confirmation Toggle */}
            <label className="flex items-start gap-4 p-4 bg-primary-50 rounded-2xl border border-primary-100 cursor-pointer group hover:bg-primary-100 transition-all">
              ...
              <div className="space-y-1">
                <p className="text-[11px] font-black text-primary-900 uppercase tracking-wider">I understand this is Pickup-Only</p>
                <p className="text-[9px] font-bold text-primary-600 tracking-tight leading-relaxed">
        ...
        <div className="p-6 sm:p-8 bg-slate-50 border-t border-slate-100 shrink-0">
          <button
            form="inquiry-form"
            type="submit"
            className="w-full py-4 bg-slate-900 text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.3em] shadow-xl hover:bg-primary-600 transition-all transform active:scale-[0.98]"
          >
