...
const BusinessTaxComplianceSettings = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setRequestType(next.currentRequest?.requestType === 'correction_resubmission' ? 'business_update' : (next.currentRequest?.requestType || (source.tax?.verificationStatus === 'verified' ? 'business_update' : 'initial_verification')));
  ...
  const status = data?.compliance?.status || profile.tax?.verificationStatus || 'unverified';
  const statusTone = status === 'restricted' ? 'border-rose-200 bg-rose-50 text-rose-800' : status === 'verified' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-800';
  ...
  return <div className="space-y-4">
    ...
    {!editing && <section className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900"><h4 className="text-xs font-black text-slate-900 dark:text-white">Current tax profile</h4><dl className="mt-3 grid gap-3 sm:grid-cols-2">{[['BIR status', labelize(profile.tax?.birRegistrationStatus)], ['TIN', profile.tax?.tinMasked || 'Not provided'], ['Branch code', profile.tax?.branchCode || 'Not provided'], ['Declared status', labelize(profile.tax?.declaredTaxStatus)], ['Verified status', labelize(profile.tax?.verifiedTaxStatus)], ['Verification', labelize(profile.tax?.verificationStatus)]].map(([label, value]) => <div key={label}><dt className="text-[8px] font-black uppercase text-slate-400">{label}</dt><dd className="mt-1 text-[11px] font-semibold text-slate-800 dark:text-slate-200">{value || 'Not provided'}</dd></div>)}</dl></div>
    </section>}
