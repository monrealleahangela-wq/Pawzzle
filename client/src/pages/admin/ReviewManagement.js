                                        ...
                                        {review.images && review.images.length > 0 && (
                                            <div className="flex gap-2 pb-2">
                                                {review.images.map((img, i) => (
                                                    <div key={i} className="w-16 h-16 sm:w-24 sm:h-24 rounded-xl bg-slate-50 border border-slate-100 overflow-hidden shrink-0 group-hover:scale-105 transition-transform duration-500">
                                                ...
                                                {replyingTo === review._id ? (
                                                    <div className="space-y-3 animate-in fade-in slide-in-from-top-4">
                                                        ...
                                                        <div className="flex gap-3">
                                                            <button
                                                                ...
                                                                className="px-8 py-3 bg-slate-900 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-primary-600 transition-all disabled:opacity-50"
                                                            >
                                                                {submittingReply ? 'Sending...' : 'Transmit Reply'}
                                                            </button>
                                                            <button
                                                                onClick={() => setReplyingTo(null)}
                                                                className="px-8 py-3 bg-slate-100 text-slate-400 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all"
                                                            >
                                                                Cancel
                                                            </button>
                                                        </div>
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-3">
                                                        <button
                                                            ...
                                                            className="flex items-center gap-2 px-6 py-3 bg-slate-50 text-slate-500 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-primary-600 hover:text-white transition-all shadow-sm"
                                                        >
                                                            <Reply className="h-4 w-4" /> Reply to Review
                                                        </button>
                                                        <button
                                                            onClick={() => handleToggleStatus(review._id)}
                                                            className={`flex items-center gap-2 px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm ${review.isApproved ? 'bg-slate-50 text-slate-500 hover:bg-rose-500 hover:text-white' : 'bg-rose-50 text-rose-500 hover:bg-emerald-500 hover:text-white'}`}
                                                        >
            ...
            {pagination.totalPages > 1 && (
                <div className="flex justify-center items-center gap-4 pt-10">
                    <button
                        ...
                        className="p-4 bg-white rounded-xl border border-slate-100 text-slate-400 disabled:opacity-30 hover:bg-primary-50 hover:text-primary-600 transition-all"
                    >
                    ...
                    <div className="flex gap-3">
                        {[...Array(pagination.totalPages)].map((_, i) => (
                            <button
                                ...
                                className={`w-12 h-12 rounded-xl text-[11px] font-black tracking-widest transition-all ${filter.page === i + 1 ? 'bg-slate-900 text-white shadow-2xl shadow-slate-200' : 'bg-white text-slate-400 border border-slate-100 hover:bg-slate-50'
                                    }`}
                            >
                                {i + 1}
                            </button>
