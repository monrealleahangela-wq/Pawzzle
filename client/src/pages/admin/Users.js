      ...
      {users.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-1.5">
          ...
          <div className="space-y-1.5">
            {users.map((user) => (
              <div key={user._id} className="group bg-white hover:bg-slate-50 border border-slate-100 rounded-2xl sm:rounded-2xl p-3 sm:p-4 transition-all duration-300">
                <div className="grid grid-cols-1 lg:grid-cols-12 items-center gap-3 sm:gap-4">
                  {/* User */}
                  <div className="lg:col-span-4 flex items-center gap-3">
                    <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-2xl flex items-center justify-center border transition-colors ${user.role === 'super_admin' ? 'bg-slate-900 border-slate-900 text-white' :
          ...
          {pagination.totalPages > 1 && (
            <div className="flex justify-between items-center bg-slate-50 p-2 rounded-2xl sm:rounded-2xl border border-slate-100 mt-6">
              <button
