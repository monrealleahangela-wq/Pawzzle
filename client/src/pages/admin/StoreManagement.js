    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
        <div className="bg-white p-12 rounded-[3rem] shadow-2xl text-center max-w-lg">
          ...
          <button onClick={handleInitialize} className="w-full py-3 bg-slate-900 text-white rounded-xl font-black uppercase">Create Store Profile</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-20">
      <div className="relative h-72 w-full overflow-hidden">
        ...
        <label className="absolute top-8 right-8 p-3 bg-white/10 backdrop-blur-md text-white rounded-xl border border-white/20 cursor-pointer hover:bg-white/20 transition-all">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-24 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white/80 backdrop-blur-xl rounded-xl shadow-2xl border border-white p-8 relative overflow-hidden">
              <div className="relative flex flex-col items-center">
                <div className="relative mb-6">
                  ...
                  <label className="absolute -right-2 -bottom-2 p-3 bg-slate-900 text-white rounded-xl shadow-lg cursor-pointer">
            ...
            <nav className="bg-white/60 backdrop-blur-md rounded-xl shadow-xl border border-white/50 p-4 space-y-2">
              {[
                ...
                <button key={item.id} onClick={() => setActiveTab(item.id)}
                  className={`w-full flex items-center gap-4 px-6 py-3 rounded-[2rem] font-bold transition-all ${activeTab === item.id ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-200' : 'text-slate-500 hover:bg-white hover:text-slate-900'}`}>
                  <item.icon className="h-5 w-5" />
                  {item.label}
                </button>
              ))}
              <div className="pt-4 border-t border-slate-100 mt-4 px-4">
                <button onClick={handleSave} disabled={saving} className="w-full flex items-center justify-center gap-3 px-6 py-3 bg-slate-900 text-white rounded-[1.8rem] font-black uppercase tracking-widest text-xs hover:bg-black transition-all disabled:opacity-50">
                ...
                <div className="space-y-12 animate-in fade-in slide-in-from-right-12 duration-700">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-2">
                      ...
                      <input type="text" name="name" value={store.name || ''} onChange={handleInputChange} className="w-full px-6 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl focus:border-indigo-500 focus:bg-white outline-none font-bold transition-all" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Business Type</label>
                      <select name="businessType" value={store.businessType || ''} onChange={handleInputChange} className="w-full px-6 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl focus:border-indigo-500 focus:bg-white outline-none font-bold transition-all appearance-none">
                    ...
                    <div className="md:col-span-2 space-y-2">
                      ...
                      <textarea name="description" value={store.description || ''} onChange={handleInputChange} rows={5} className="w-full px-6 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl focus:border-indigo-500 focus:bg-white outline-none font-bold transition-all resize-none" placeholder="Tell customers about your pet shop..."></textarea>
                    </div>
                    {/* Location Intel */}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div className="space-y-2">
                          ...
                          <select
                            ...
                            className="w-full px-6 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl focus:border-indigo-500 focus:bg-white outline-none font-bold transition-all appearance-none"
                          >
                        ...
                        <div className="space-y-2">
                          ...
                          <select
                            ...
                            className="w-full px-6 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl focus:border-indigo-500 focus:bg-white outline-none font-bold transition-all appearance-none disabled:opacity-50 cursor-pointer"
                          >
                        ...
                        <div className="md:col-span-3 space-y-2">
                          ...
                          <input
                            ...
                            className="w-full px-6 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl focus:border-indigo-500 focus:bg-white outline-none font-bold transition-all"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {['facebook', 'instagram', 'twitter', 'youtube'].map(s => (
                      <div key={s} className="space-y-2">
                        ...
                        <input type="text" value={store.socialMedia?.[s] || ''} onChange={(e) => handleSocialChange(s, e.target.value)} className="w-full px-6 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl focus:border-indigo-500 focus:bg-white outline-none font-bold transition-all" />
                      </div>
                    ))}
                  </div>
                </div>
