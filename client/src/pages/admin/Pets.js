  ...
  return (
    <div className="min-h-screen bg-slate-50/50 p-4 sm:p-8 space-y-8">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 bg-white p-6 sm:p-10 rounded-[3rem] border border-slate-100 shadow-sm relative overflow-hidden">
        ...
        <button onClick={handleOpenModal} className="px-8 py-3 bg-slate-900 text-white rounded-xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-rose-500 transition-all shadow-xl shadow-slate-200 flex items-center gap-3 group">
          <Plus className="h-4 w-4 group-hover:rotate-90 transition-transform" /> Add New Pet
        </button>
      </div>

      {/* View Switcher */}
      <div className="flex gap-4 p-1.5 bg-white rounded-xl border border-slate-100 shadow-sm w-fit">
        <button
          onClick={() => setActiveTab('inventory')}
          className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center gap-3 ${activeTab === 'inventory' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
        >
          <PawPrint className="h-4 w-4" /> Pet Inventory
        </button>
        <button
          onClick={() => setActiveTab('adoptions')}
          className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center gap-3 ${activeTab === 'adoptions' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
        >
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm space-y-5">
              ...
              <div className="input-container group">
                ...
                <input
                  ...
                  className="input input-with-icon bg-slate-50 border-slate-100 rounded-xl text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-primary-600/10 placeholder:text-slate-300"
                />
              </div>
              ...
              {[
                ...
                <div key={field} className="space-y-2">
                  ...
                  <select value={filters[field]} onChange={e => handleFilterChange(field, e.target.value)}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-black uppercase tracking-widest outline-none appearance-none focus:ring-2 focus:ring-primary-600/10">
              ...
              <button onClick={() => setFilters({ species: '', size: '', gender: '', isAvailable: '', search: '' })}
                className="w-full py-3 bg-slate-50 hover:bg-slate-100 text-slate-400 text-[10px] font-black uppercase tracking-widest rounded-xl transition-colors border border-slate-100">
                Reset Filters
              </button>
            </div>
          </aside>
          ...
          <div className="flex-1 space-y-6">
            {pets.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4">
                {pets.map((pet) => (
                  <div key={pet._id} className="group bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-0.5 transition-all overflow-hidden flex flex-col">
                    {/* Image */}
            ...
            {pagination.totalPages > 1 && (
              <div className="flex justify-center items-center gap-6 bg-white border border-slate-100 p-4 rounded-xl w-fit mx-auto shadow-sm">
            ...
            <div className="flex items-center gap-3">
              <div className="input-container group">
                ...
                <input
                  ...
                  className="input input-with-icon bg-slate-50 border-slate-100 rounded-xl text-[10px] font-black uppercase outline-none focus:ring-2 focus:ring-rose-500/10 w-64 placeholder:text-slate-300"
                />
              </div>
            </div>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50/50">
                  <th className="px-8 py-3 text-left text-[9px] font-black text-slate-400 uppercase tracking-widest">Biological Subject</th>
                  <th className="px-8 py-3 text-left text-[9px] font-black text-slate-400 uppercase tracking-widest">Customer Profile</th>
                  <th className="px-8 py-3 text-left text-[9px] font-black text-slate-400 uppercase tracking-widest">Status / Lifecycle</th>
                  <th className="px-8 py-3 text-left text-[9px] font-black text-slate-400 uppercase tracking-widest">Timestamp</th>
                ...
                {fetchingAdoptions ? (
                  Array(5).fill(0).map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      <td colSpan={5} className="px-8 py-6"><div className="h-12 bg-slate-50 rounded-xl w-full" /></td>
                    </tr>
                  ))
          ...
          <div className="bg-white w-full max-w-5xl rounded-[3rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
            <header className="p-8 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
              <div className="flex items-center gap-5">
                <div className="w-14 h-14 bg-rose-500 rounded-xl flex items-center justify-center shadow-2xl shadow-rose-200">
              ...
              <button onClick={() => { setShowAddForm(false); setEditingPet(null); }} className="p-4 bg-slate-50 text-slate-400 rounded-xl hover:bg-rose-50 hover:text-rose-600 transition-all"><X className="h-6 w-6" /></button>
            </header>
            ...
            <nav className="px-8 py-3 border-b border-slate-50 flex gap-4 overflow-x-auto no-scrollbar shrink-0 bg-slate-50/50">
              {[
                ...
                <button key={tab.id} onClick={() => setModalTab(tab.id)}
                  className={`px-6 py-3 rounded-xl flex items-center gap-3 text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${modalTab === tab.id ? 'bg-white text-rose-500 shadow-sm border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}>
                ...
                <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-500">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="text" required value={petForm.name} onChange={e => setPetForm(p => ({ ...p, name: e.target.value }))}
                      className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-xl text-[13px] font-black uppercase outline-none focus:ring-2 focus:ring-rose-400/20" placeholder="PET NAME..." />
                  </div>
                  {/* Row 2: Species + Breed */}
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <select value={petForm.species} onChange={e => setPetForm(p => ({ ...p, species: e.target.value }))}
                        className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-[11px] font-black uppercase outline-none appearance-none cursor-pointer">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input type="text" required value={petForm.breed} onChange={e => setPetForm(p => ({ ...p, breed: e.target.value }))}
                        className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-xl text-[11px] font-black uppercase outline-none" placeholder="E.G. GOLDEN RETRIEVER" />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-12 gap-4">
                    <div className="col-span-6 space-y-1.5">
                      ...
                      <input type="number" required value={petForm.price} onChange={e => setPetForm(p => ({ ...p, price: e.target.value }))}
                        className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-xl text-lg font-black outline-none" placeholder="0.00" />
                    </div>
                    <div className="col-span-3 space-y-1.5">
                      ...
                      <input type="number" required value={petForm.age} onChange={e => setPetForm(p => ({ ...p, age: e.target.value }))}
                        className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-lg font-black outline-none text-center" placeholder="0" />
                    </div>
                    <div className="col-span-3 space-y-1.5">
                      ...
                      <select value={petForm.ageUnit} onChange={e => setPetForm(p => ({ ...p, ageUnit: e.target.value }))}
                        className="w-full px-3 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase outline-none appearance-none cursor-pointer text-center">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="text" value={petForm.color} onChange={e => setPetForm(p => ({ ...p, color: e.target.value }))}
                      className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-xl text-[11px] font-black uppercase outline-none" placeholder="E.G. GOLDEN, BLACK, TRI-COLOR..." />
                  </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {['male', 'female'].map(g => (
                          <button key={g} type="button" onClick={() => setPetForm(p => ({ ...p, gender: g }))}
                            className={`py-3 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all border-2 ${petForm.gender === g ? 'bg-rose-500 border-rose-400 text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-400 hover:border-rose-200'}`}>
                            {g}
                          </button>
                        ))}
                      </div>
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {[{ v: 'small', l: 'S' }, { v: 'medium', l: 'M' }, { v: 'large', l: 'L' }, { v: 'extra_large', l: 'XL' }].map(s => (
                          <button key={s.v} type="button" onClick={() => setPetForm(p => ({ ...p, size: s.v }))}
                            className={`py-3 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all border-2 ${petForm.size === s.v ? 'bg-slate-900 border-slate-800 text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-400 hover:border-slate-300'}`}>
                            {s.l}
                          </button>
                        ))}
                      </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea required value={petForm.description} onChange={e => setPetForm(p => ({ ...p, description: e.target.value }))}
                      className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-xl text-[12px] font-medium leading-relaxed outline-none h-28 resize-none" placeholder="Tell customers about this pet..." />
                  </div>
                </div>
              )}
                      ...
                      <div className="space-y-4">
                        {[
                          ...
                          <button key={opt.v} type="button" onClick={() => setPetForm(p => ({ ...p, vaccinationStatus: opt.v }))}
                            className={`w-full p-5 rounded-xl text-left transition-all border-2 ${petForm.vaccinationStatus === opt.v ? 'bg-white/20 border-white shadow-xl' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                      ...
                      <div className="space-y-4">
                        {[
                          ...
                          <button key={opt.v} type="button" onClick={() => setPetForm(p => ({ ...p, healthStatus: opt.v }))}
                            className={`w-full p-5 rounded-xl text-left transition-all border-2 ${petForm.healthStatus === opt.v ? 'bg-white/20 border-primary-500 shadow-xl' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                            <p className="text-[11px] font-black uppercase tracking-widest mb-1">{opt.l}</p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <textarea value={petForm.specialNeeds} onChange={e => setPetForm(p => ({ ...p, specialNeeds: e.target.value }))}
                        className="w-full px-8 py-6 bg-slate-50 border border-slate-100 rounded-xl text-[12px] font-medium h-36 resize-none outline-none shadow-inner" placeholder="Allergies, diet, or behavior notes..." />
                    </div>
                    <div className={`${petForm.status === 'adopted' ? 'bg-rose-500' : petForm.status === 'reserved' ? 'bg-amber-500' : 'bg-emerald-500'} p-8 rounded-xl flex items-center justify-between shadow-xl self-start transition-colors`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        className="w-full px-6 py-3 bg-slate-50 border border-slate-100 rounded-xl text-[12px] h-32 resize-none outline-none focus:ring-2 focus:ring-rose-500/10"
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-4">
                        <input
                          ...
                          className="w-full px-6 py-3 bg-slate-50 border border-slate-100 rounded-xl text-[11px] font-black uppercase outline-none"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                      ...
                      <button
                        ...
                        className={`p-6 rounded-xl border-2 transition-all flex flex-col items-center gap-3 ${petForm.adoptionDetails?.[toggle.id]
                ...
                <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                  <div className="p-10 bg-slate-900 rounded-[3rem] text-white relative overflow-hidden shadow-2xl border border-white/5">
                    ...
                    <div className="flex items-center gap-4 mb-8">
                      <div className="p-3 bg-rose-500 rounded-xl"><ImageIcon className="h-5 w-5" /></div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {petForm.images.map((img, i) => (
                        <div key={i} className="aspect-square bg-white/5 rounded-xl overflow-hidden border border-white/10 relative group">
                      ...
                      {petForm.images.length < 10 && (
                        <label className="aspect-square bg-white/5 rounded-xl border-2 border-white/10 border-dashed flex flex-col items-center justify-center cursor-pointer hover:bg-white/10 transition-all hover:border-rose-500 group">
            ...
            <footer className="p-8 bg-white border-t border-slate-50 flex gap-4 shrink-0 relative z-20">
              <button type="button" onClick={() => { setShowAddForm(false); setEditingPet(null); }}
                className="px-10 py-3 bg-slate-50 text-slate-400 rounded-xl text-[11px] font-black uppercase tracking-widest hover:bg-slate-100 transition-all active:scale-95">
                Cancel
              </button>
              <button disabled={submitting} type="button"
                ...
                className="flex-1 py-3 bg-rose-500 text-white rounded-xl text-[11px] font-black uppercase tracking-widest hover:bg-slate-900 hover:scale-[1.02] transition-all shadow-2xl shadow-rose-200 flex items-center justify-center gap-4 active:scale-95 disabled:opacity-50">
                {submitting ? 'Saving...' : (
