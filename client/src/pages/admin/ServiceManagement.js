    ...
    <div className="min-h-screen bg-slate-50/50 p-4 sm:p-8 space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={idx} className="bg-white border border-slate-100 rounded-xl p-5 shadow-sm group hover:shadow-lg hover:border-indigo-100 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {services
          ...
          .map((service) => (
          <div key={service._id} className="group bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-0.5 transition-all relative flex flex-col">
            ...
            <div className="p-4 flex-1 flex flex-col">
              ...
              {/* Stats row */}
              <div className="grid grid-cols-3 gap-1 py-3 border-t border-slate-50 mt-auto">
              ...
              <div className="flex gap-2 mt-3">
                <button onClick={() => handleEdit(service)}
                  className="flex-1 py-2.5 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all hover:bg-indigo-600 flex items-center justify-center gap-1.5">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4 overflow-hidden">
          <div className="bg-white w-full max-w-5xl rounded-[3rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
            <header className="p-8 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
              ...
              <button onClick={() => setShowModal(false)} className="p-4 bg-slate-50 text-slate-400 rounded-xl hover:bg-rose-50 hover:text-rose-600 transition-all"><X className="h-6 w-6" /></button>
            </header>
            ...
            <nav className="px-8 py-3.5 border-b border-slate-50 flex gap-4 overflow-x-auto no-scrollbar shrink-0 bg-slate-50/50">
              {[
                ...
                <button key={tab.id} onClick={() => setModalTab(tab.id)}
                  className={`px-6 py-3 rounded-xl flex items-center gap-3 text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${modalTab === tab.id ? 'bg-white text-indigo-600 shadow-sm border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {categories.map(c => (
                            <button key={c.value} type="button" onClick={() => setFormData(p => ({ ...p, category: c.value }))}
                              className={`p-4 rounded-xl text-left transition-all border-2 flex items-center gap-3 ${formData.category === c.value ? 'bg-indigo-50 border-indigo-300 shadow-lg' : 'bg-slate-50 border-slate-50 hover:border-slate-200'}`}>
                        ...
                        <div className="flex flex-wrap gap-2 mb-3">
                          {durationPresets.map(d => (
                            <button key={d} type="button" onClick={() => setFormData(p => ({ ...p, duration: d }))}
                              className={`px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border-2 ${formData.duration === d ? 'bg-indigo-600 border-indigo-500 text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-500 hover:border-indigo-200'}`}>
                              {d >= 60 ? `${d / 60}h` : `${d}m`}
                            </button>
                          ))}
                        </div>
                  ...
                  <div className="bg-indigo-600 rounded-[3rem] p-10 text-white relative overflow-hidden shadow-2xl">
                    ...
                    <div className="flex items-center justify-between mb-8">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 bg-white/10 rounded-xl flex items-center justify-center"><Home className="h-7 w-7" /></div>
                        <div>
                ...
                <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
                  ...
                  <div className="p-10 bg-slate-900 rounded-[3rem] text-white relative overflow-hidden shadow-2xl border border-white/5">
                    ...
                    <div className="flex items-center gap-4 mb-8">
                      <div className="p-3 bg-indigo-600 rounded-xl"><ImageIcon className="h-5 w-5" /></div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      {formData.images.length < 6 && (
                        <label className="aspect-square bg-white/5 rounded-xl border-2 border-white/10 border-dashed flex flex-col items-center justify-center cursor-pointer hover:bg-white/10 transition-all hover:border-indigo-500 group">
