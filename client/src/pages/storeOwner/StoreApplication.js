...
import { Upload, FileText, AlertCircle, Building, Phone, Mail, MapPin, Check, X, Shield, Users, ArrowRight, ChevronRight, Briefcase, Globe, Info, Wallet, Camera, Truck, Zap, Heart, Calendar, TrendingUp, Package } from 'lucide-react';
...
const StoreApplication = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    staffingConfiguration: {
      hasStaff: 'no',
      staffQuantityRange: '1-5',
      isStaffVisibleToCustomers: false,
      ...
      <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in">
        ...
        {currentStep === 2 && (
          <div className="card p-10 space-y-8 animate-slide-up">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-6">
                <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em] flex items-center gap-2">
                  <Users className="h-3 w-3 text-primary-500" /> Professional Workforce
                </h3>
                <div className="space-y-4">
                  <p className="text-[10px] font-black text-slate-500 uppercase">Operational Protocol</p>
                  <p className="text-[9px] text-slate-400 font-bold uppercase italic leading-relaxed">
                    Will your business operate with staff members or employees?
                  </p>
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      { id: 'yes', label: 'DEPLOYED' },
                      { id: 'no', label: 'SOLO OP' },
                      { id: 'planning', label: 'SCALING SOON' }
                    ].map(opt => (
                      <button
                        key={opt.id}
                        type="button"
                        onClick={() => setFormData(prev => ({
                          ...prev,
                          staffingConfiguration: { ...prev.staffingConfiguration, hasStaff: opt.id }
                        }))}
                        className={`p-3 rounded-xl border-2 text-[10px] font-black transition-all ${formData.staffingConfiguration.hasStaff === opt.id ? 'border-primary-600 bg-primary-50' : 'border-slate-100 hover:border-primary-200'}`}
                      >
                        {opt.label}
                      </button>
                    ))}
                ...
                {formData.staffingConfiguration.hasStaff !== 'no' && (
                  <div className="space-y-6 animate-in slide-in-from-top duration-300">
                    <div className="space-y-3">
                      <p className="text-[10px] font-black text-slate-500 uppercase">Staff Quantity range</p>
                      <div className="flex gap-2">
                        {['1-5', '6-15', '16+'].map(q => (
                          <button
                            key={q}
                            ...
                            onClick={() => setFormData(prev => ({
                              ...prev,
                              staffingConfiguration: { ...prev.staffingConfiguration, staffQuantityRange: q }
                            }))}
                            className={`flex-1 p-2 rounded-lg border text-[10px] font-bold transition-all ${formData.staffingConfiguration.staffQuantityRange === q ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-400 border-slate-100'}`}
                          >
                            {q}
                          </button>
                        ))}
                    ...
                    <div className="space-y-4 p-5 bg-slate-50 rounded-2xl border border-slate-100">
                      <div className="flex items-center justify-between">
                        <label className="text-[10px] font-black text-slate-900 uppercase">Visible to Customers?</label>
                        <button
                          type="button"
                          onClick={() => setFormData(prev => ({
                            ...prev,
                            staffingConfiguration: { ...prev.staffingConfiguration, isStaffVisibleToCustomers: !prev.staffingConfiguration.isStaffVisibleToCustomers }
                          }))}
                          className={`w-10 h-5 rounded-full transition-all relative ${formData.staffingConfiguration.isStaffVisibleToCustomers ? 'bg-emerald-500' : 'bg-slate-200'}`}
                        >
                          <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${formData.staffingConfiguration.isStaffVisibleToCustomers ? 'left-6' : 'left-1'}`} />
                        </button>
                      </div>
                      <div className="flex items-center justify-between">
                        <label className="text-[10px] font-black text-slate-900 uppercase">Verification Required?</label>
                        <button
                          type="button"
                          onClick={() => setFormData(prev => ({
                            ...prev,
                            staffingConfiguration: { ...prev.staffingConfiguration, requiresProfessionalVerification: !prev.staffingConfiguration.requiresProfessionalVerification }
                          }))}
                          className={`w-10 h-5 rounded-full transition-all relative ${formData.staffingConfiguration.requiresProfessionalVerification ? 'bg-primary-600' : 'bg-slate-200'}`}
                        >
                          <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${formData.staffingConfiguration.requiresProfessionalVerification ? 'left-6' : 'left-1'}`} />
                        </button>
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-6">
                <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Role Classifications</h3>
                <div className="space-y-6">
                  <div>
                    <p className="text-[9px] font-black text-primary-600 uppercase mb-3">Service Professionals</p>
                    <div className="grid grid-cols-2 gap-2">
                       {[
                         ...
                         { id: 'trainers', label: 'Trainers' },
                         { id: 'boarding_specialists', label: 'Boarding' },
                         { id: 'medical_assistants', label: 'Med Assistants' },
                         { id: 'pet_handlers', label: 'Handlers' }
                       ].map(role => (
                         <button
                           key={role.id}
                           type="button"
                           onClick={() => {
                             const curr = formData.staffTypes || [];
                             const next = curr.includes(role.id) ? curr.filter(i => i !== role.id) : [...curr, role.id];
                             handleChange('staffTypes', next);
                           }}
                           className={`px-3 py-2 rounded-lg text-[8px] font-black uppercase text-left border transition-all ${formData.staffTypes.includes(role.id) ? 'bg-primary-600 border-primary-600 text-white' : 'bg-white border-slate-100 text-slate-400 hover:border-primary-100'}`}
                         >
                            {role.label}
                         </button>
                       ))}
                  ...
                  <div>
                    <p className="text-[9px] font-black text-secondary-600 uppercase mb-3">Operational Support</p>
                    <div className="grid grid-cols-2 gap-2">
                       {[
                         { id: 'inventory', label: 'Inventory' },
                         { id: 'logistics', label: 'Logistics' },
                         { id: 'sales', label: 'Sales' },
                         { id: 'service_mgmt', label: 'Services Mgmt' },
                         ...
                         <button
                           ...
                           className={`px-3 py-2 rounded-lg text-[8px] font-black uppercase text-left border transition-all ${formData.staffTypes.includes(role.id) ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-100 text-slate-400 hover:border-slate-200'}`}
                         >
                            {role.label}
                         </button>
                       ))}
        ...
        {currentStep > 0 && (
          <div className="flex justify-between items-center bg-white/50 backdrop-blur-md p-2 rounded-full border border-slate-100">
            ...
            <div className="flex gap-2">
              {[0, 1, 2, 3, 4, 5].map(s => <div key={s} className={`w-2 h-2 rounded-full transition-all duration-300 ${currentStep === s ? 'bg-primary-600 w-6' : 'bg-slate-200'}`} />)}
            </div>
            {currentStep < 5 ? (
