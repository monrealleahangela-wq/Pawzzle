...
const Pets = () => {
  ...
  return (
    <div className="customer-marketplace-page marketplace-catalog marketplace-pets w-full max-w-full min-w-0 space-y-4 sm:space-y-8 animate-fade-in pb-20 px-1 sm:px-0">
      ...
      <div className="flex flex-col space-y-4">
        <div className="marketplace-page-header flex min-w-0 flex-col md:flex-row justify-between items-start md:items-end gap-3 sm:gap-6 bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
          ...
          <form onSubmit={handleSearch} className="marketplace-page-search flex w-full min-w-0 flex-col gap-2 sm:flex-row md:max-w-sm">
      ...
      <div className="marketplace-catalog-layout flex w-full min-w-0 flex-col gap-4 xl:flex-row xl:items-start xl:gap-5">
        ...
        <aside className="marketplace-filter-sidebar w-full min-w-0 xl:w-60 xl:shrink-0">
          ...
          <div className={`${showMobileFilters ? 'block' : 'hidden xl:block'} marketplace-filter-panel card xl:sticky xl:top-24 p-4 border-slate-100 bg-white dark:bg-slate-900 dark:border-slate-800 shadow-xl animate-fade-in`}>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest ml-1">Pet Type</label>
                <select
          ...
          {pets.length === 0 ? (
            ...
            <div className="marketplace-result-grid responsive-card-grid [--card-min:17rem] sm:[--card-min:18rem] xl:[--card-min:19rem]">
              {pets.map((pet, idx) => (
                <div
                  key={pet._id}
                  className="marketplace-listing-card group w-full min-w-0 max-w-full bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 p-2 flex flex-col transition-all hover:shadow-2xl hover:shadow-primary-100/10 relative overflow-hidden animate-slide-up"
                    ...
                    <div className="absolute top-3 right-3 flex flex-col gap-1.5">
                      ...
                      {pet.store?.verificationStatus === 'verified' && (
                        <span className="px-2.5 py-1 bg-slate-900/90 backdrop-blur-md rounded-xl text-[8px] font-black uppercase tracking-widest text-white border border-white/10 shadow-sm flex items-center gap-1">
                          <Star className="h-2 w-2 text-secondary-400 fill-current" />
                          VERIFIED
                        </span>
                      )}
                    </div>
                      ...
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-5 h-5 rounded-lg bg-slate-50 overflow-hidden flex items-center justify-center shrink-0 group-hover/store:bg-primary-50 transition-colors border border-slate-100">
                          {pet.store?.logo ? (
                            ...
                            <Store className="h-2.5 w-2.5 text-primary-500" />
                          )}
                        </div>
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter truncate group-hover/store:text-primary-600 transition-colors">{pet.store?.name}</span>
                      </div>
                      <p className="text-[8px] font-black text-primary-600 uppercase tracking-widest opacity-0 group-hover/store:opacity-100 transition-all">Visit →</p>
                    </Link>
                  </div>
                </div>
              ))}
