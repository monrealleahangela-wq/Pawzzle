...
const EmployeeHR = ({ initialTab = 'attendance' }) => {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl p-4 sm:p-6 text-slate-900 dark:text-slate-100">
      ...
      <nav className="content-scroll-row mb-5 gap-2 border-b border-slate-200 dark:border-slate-800">
        {tabs.map(tab => <Link key={tab.id} to={tab.path} className={`flex shrink-0 items-center gap-2 whitespace-nowrap border-b-2 px-3 py-2 text-sm font-semibold ${initialTab === tab.id ? 'border-primary text-primary' : 'border-transparent text-slate-500'}`}><tab.icon size={16} />{tab.label}</Link>)}
      </nav>
