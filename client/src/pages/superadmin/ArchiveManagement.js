        ...
        <div className="space-y-6 pb-20">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3 border-b border-slate-100 pb-4">
                ...
                <div className="flex items-center gap-3">
                    <div className="px-4 py-2 bg-slate-900 rounded-xl flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
                    <div className="md:col-span-12 relative group">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-32 py-3 bg-slate-800 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-xl outline-none focus:ring-2 focus:ring-primary-500/50 placeholder:text-slate-600 transition-all font-sans"
                        />
                        <button 
                            onClick={handleSearch}
                            className="absolute right-3 top-1/2 -translate-y-1/2 px-6 py-2.5 bg-primary-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-primary-500 transition-all active:scale-95"
                        >
                            Execute
                        </button>
                    </div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
                <button
                    ...
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest whitespace-nowrap shrink-0 transition-all border-2 ${activeType === 'all'
                ...
                {Object.entries(typeConfig).map(([key, cfg]) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest whitespace-nowrap shrink-0 transition-all border-2 ${activeType === key
                                ? 'bg-slate-900 border-slate-900 text-white shadow-lg'
                                : 'bg-white border-slate-100 text-slate-700 hover:border-slate-300'
                                }`}
                        >
                ...
                <div className="space-y-6">
                    {totalArchived === 0 ? (
                        <div className="text-center py-20 bg-white border border-slate-100 rounded-xl">
                            <div className="w-16 h-16 bg-slate-50 rounded-xl flex items-center justify-center text-slate-200 mx-auto mb-4">
                        ...
                        Object.entries(allResults).map(([type, typeItems]) => {
                            ...
                            const Icon = cfg.icon;
                            return (
                                <div key={type} className="bg-white border border-slate-100 rounded-xl overflow-hidden">
                                    ...
                                    <div className="divide-y divide-slate-50">
                                        {typeItems.slice(0, 5).map(item => (
                                            <div key={item._id} className="flex items-center justify-between px-5 sm:px-6 py-3 sm:py-3 hover:bg-slate-50/50 transition-colors">
            ...
            {activeType !== 'all' && (
                <div className="bg-white border border-slate-100 rounded-xl overflow-hidden">
                    {items.length === 0 ? (
                        ...
                        <div className="divide-y divide-slate-50">
                            {items.map(item => (
                                <div key={item._id} className="flex items-center gap-4 px-5 sm:px-8 py-3 sm:py-3 hover:bg-slate-50/50 transition-colors group">
                                    <div className={`w-10 h-10 rounded-xl bg-${typeConfig[activeType]?.color}-50 flex items-center justify-center shrink-0`}>
                    ...
                    <div className="bg-white rounded-[3rem] max-w-md w-full shadow-2xl relative overflow-hidden">
                        ...
                        <div className="flex gap-3 p-6 bg-slate-50 border-t border-slate-100">
                            <button
                                ...
                                className="flex-1 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-100 transition-all"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={handlePermanentDelete}
                                className="flex-1 py-3 bg-rose-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-700 transition-all shadow-lg shadow-rose-200"
                            >
                                Delete Forever
                            </button>
                        </div>
