                ...
                <div className="space-y-6">
                    {totalArchived === 0 ? (
                        ...
                        Object.entries(allResults).map(([type, typeItems]) => {
                            ...
                            const Icon = cfg.icon;
                            return (
                                <div key={type} className="bg-white border border-slate-100 rounded-2xl overflow-hidden">
                                    ...
                                    <div className="divide-y divide-slate-50">
                                        {typeItems.slice(0, 5).map(item => (
                                            <div key={item._id} className="flex items-center justify-between px-5 sm:px-6 py-3.5 sm:py-3.5 hover:bg-slate-50/50 transition-colors">
            ...
            {activeType !== 'all' && (
                <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden">
                    {items.length === 0 ? (
                        ...
                        <div className="divide-y divide-slate-50">
                            {items.map(item => (
                                <div key={item._id} className="flex items-center gap-4 px-5 sm:px-8 py-3.5 sm:py-3.5 hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <div className="flex gap-1.5 shrink-0 opacity-60 group-hover:opacity-100 transition-opacity">
                                        <button
                                            ...
                                            className="px-3 py-2 bg-emerald-50 text-emerald-600 rounded-2xl hover:bg-emerald-600 hover:text-white transition-all text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5"
                                        >
                                            <RotateCcw className="h-3 w-3" /> Restore
                                        </button>
                                        <button
                                            onClick={() => setConfirmModal({ type: activeType, id: item._id, name: getItemName(activeType, item) })}
                                            className="px-3 py-2 bg-rose-50 text-rose-600 rounded-2xl hover:bg-rose-600 hover:text-white transition-all text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5"
                                        >
